...
# Metrics
ws_connections = Gauge('websocket_connections_active', 'Active WebSocket connections', ['region', 'node'])
    ...
    async def disconnect(self, connection_id: str, reason: str = "Normal closure"):
        ...
        try:
            ...
            await self.send_message(
                ...
                Message(
                    type=MessageType.SYSTEM,
                    content={"event": "disconnecting", "reason": reason}
                )
            )
    ...
    async def handle_incoming_message(self, connection_id: str, data: Dict[str, Any]):
        ...
        try:
            ...
            if message.type == MessageType.HEARTBEAT:
                ...
                await self.send_message(
                    connection_id,
                    Message(type=MessageType.HEARTBEAT, content="pong")
                )
                return
    ...
    async def _cleanup_loop(self):
        ...
        while True:
            try:
                ...
                empty_sessions = [
                    session_id for session_id, conns in self.session_connections.items()
                    if not conns
                ]
