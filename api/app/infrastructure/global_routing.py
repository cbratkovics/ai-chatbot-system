            ...
            try:
                ...
                response = self.route53.create_health_check(
                    ...
                    Tags=[
                        {
                            'Key': 'Name',
                            'Value': f'chatbot-{region.name}-health-check'
                        },
                        {
                            'Key': 'Region',
                            'Value': region.name
...
def lambda_handler(event, context):
    ...
    async def route_request(
        ...
        logger.info(f"Routed request to {best_region_name} (score: {region_scores[best_region_name]:.1f})")
...
class BlueGreenDeploymentManager:
    ...
    async def _blue_green_deployment(
        ...
        for region_name in regions:
            ...
            green_endpoint = await self._deploy_green_environment(
                region_name,
                version
            )
