...
class ReconnectionManager:
    ...
    async def handle_disconnection(
        ...
        logger.info(
            f"Session {session_id} disconnected (reason: {reason}), "
            f"reconnect: {reconnect}"
        )
    
    async def _reconnection_loop(
        self,
        session_id: str
    ):
        ...
        while info.attempts < self.config.max_attempts:
            ...
            try:
                ...
                logger.error(
                    f"Reconnection timeout for session {session_id} "
