...
class BackpressureController:
    ...
    async def send_message(
        ...
        if session_id not in self.rate_limiters:
            self.rate_limiters[session_id] = asyncio.Semaphore(
                int(self.initial_send_rate)
            )
    ...
    async def handle_nack(
        ...
        if self.strategy == FlowControlStrategy.ADAPTIVE:
            ...
            logger.warning(
                f"Received NACK for session {session_id}, "
                f"reducing rate to {new_rate} msg/s"
            )
    ...
    async def _adjust_congestion_window(
        self,
        session_id: str,
        metrics: FlowMetrics
    ):
        ...
        if in_flight < metrics.congestion_window:
            ...
            metrics.congestion_window = min(
                ...
                metrics.congestion_window + 1
            )
        elif metrics.rtt > 1.0:
            # High RTT indicates congestion
            metrics.congestion_window = max(
                1,
                metrics.congestion_window - 1
            )
