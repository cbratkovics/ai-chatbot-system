...
class FallbackManager:
    ...
    def _update_provider_health(self, provider: str, success: bool):
        ...
        if success:
            self.provider_health[provider] = min(
                1.0,
                self.provider_health[provider] * (1 - alpha) + alpha
            )
        else:
            self.provider_health[provider] = max(
                0.0,
                self.provider_health[provider] * (1 - alpha)
            )
    ...
    def get_fallback_stats(self) -> Dict[str, Any]:
        ...
        recent_fallbacks = [
            e for e in self.fallback_history
