...
class ProviderOrchestrator:
    ...
    def select_provider(self, request: CompletionRequest) -> Optional[BaseProvider]:
        ...
        if not compatible_providers:
            logger.warning(f"No providers support model {request.model}, using any healthy provider")
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                if attempt > 0:
                    ...
                    logger.info(f"Request succeeded after {attempt} failovers using {provider.name}")
                ...
                if not e.retryable:
                    logger.debug(f"Error is not retryable, marking provider {provider.name} as failed")
    ...
    async def complete_stream(self, request: CompletionRequest) -> AsyncIterator[StreamChunk]:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                if attempt > 0:
                    self.failover_count += 1
                    logger.info(f"Streaming request succeeded after {attempt} failovers using {provider.name}")
