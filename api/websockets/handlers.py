...
from .manager import WebSocketConnection, ConnectionManager
...
class WebSocketHandler:
    ...
    async def handle_connection(self, connection: WebSocketConnection):
        ...
        try:
            ...
            await self.connection_manager.disconnect(
                connection.id,
                reason="Connection handler finished"
            )
    ...
    async def _handle_chat_message(self, connection: WebSocketConnection, event: WebSocketEvent):
        ...
        if stream:
            ...
            await self._handle_non_streaming_request(connection, completion_request, conversation_id)
    ...
    async def _handle_non_streaming_request(
        ...
        try:
            ...
            logger.info(f"Completed non-streaming response for {connection.id} in {response.latency_ms:.2f}ms")
    ...
    async def _handle_typing_indicator(self, connection: WebSocketConnection, event: WebSocketEvent):
        ...
        if conversation_id_str:
            ...
            connection_ids = self.connection_manager.conversation_connections.get(conversation_id, set())
    ...
    async def broadcast_system_message(self, message: str, level: str = "info", tenant_id: Optional[UUID] = None):
