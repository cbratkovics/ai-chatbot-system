...
class CostTracker:
    ...
    def track_usage(
        ...
        logger.debug(f"Tracked cost: ${total_cost:.4f} for {model} ({prompt_tokens}+{completion_tokens} tokens)")
    ...
    def export_costs(self, format: str = "json") -> str:
        ...
        if format == "json":
            ...
            writer = csv.DictWriter(
                output,
                fieldnames=[
                    "timestamp", "tenant_id", "user_id", "provider",
                    "model", "tokens", "cost", "cached"
                ]
            )
