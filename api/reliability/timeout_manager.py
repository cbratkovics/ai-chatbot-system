...
class TimeoutManager:
    ...
    def _update_operation_timeout(
        ...
        if operation not in self.operation_timeouts:
            ...
            alpha = 0.1
            self.operation_timeouts[operation] = (
                (1 - alpha) * self.operation_timeouts[operation] +
                alpha * duration_ms
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        durations = [
            e.actual_duration_ms
            for e in self.timeout_events
            if e.actual_duration_ms is not None
        ]
        ...
        for event in self.timeout_events:
            if event.operation not in operation_stats:
                operation_stats[event.operation] = {
                    "total": 0,
                    "timeouts": 0,
                    "durations": []
                }
            ...
            if event.actual_duration_ms:
                operation_stats[event.operation]["durations"].append(
...
def timeout(
    timeout_ms: float,
    operation: str = "decorated_operation",
    propagate_deadline: bool = True
):
