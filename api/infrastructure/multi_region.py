...
failover_events = Counter('failover_events_total', 'Total failover events', ['from_region', 'to_region'])
...
class MultiRegionManager:
    ...
    async def _check_all_regions_health(self):
        ...
        tasks = [
            self._check_region_health(region_name)
    ...
    async def _perform_failover(self):
        ...
        try:
            ...
            if failover_time > self.config.rto_seconds:
                logger.warning(f"Failover exceeded RTO: {failover_time:.1f}s > {self.config.rto_seconds}s")
        ...
        for pattern in replication_patterns:
            try:
                ...
                for region_name, redis_conn in self.redis_connections.items():
                    ...
                    asyncio.create_task(
                        self._replicate_keys(primary_redis, redis_conn, keys)
                    )
                    
            except Exception as e:
                logger.error(f"Replication failed for pattern {pattern}: {e}")
                
    async def _replicate_keys(self, source: aioredis.Redis, target: aioredis.Redis, keys: List[str]):
    ...
    async def get_redis_connection(self, prefer_region: Optional[str] = None) -> Tuple[str, aioredis.Redis]:
