...
class ModelRouter:
    ...
    def _score_models(
        self, complexity: QueryComplexity, strategy: RoutingStrategy, max_tokens: int
    ) -> List[ModelScore]:
    ...
    async def _route_round_robin(self, model_config: ModelConfig, complexity: QueryComplexity) -> RoutingDecision:
        """Round-robin routing across available providers."""
        available_providers = [(name, provider) for name, provider in self.providers.items() if provider.is_available()]
    ...
    async def _route_least_latency(self, model_config: ModelConfig, complexity: QueryComplexity) -> RoutingDecision:
    ...
    async def _route_failover(self, model_config: ModelConfig, complexity: QueryComplexity) -> RoutingDecision:
        ...
        for provider_name in priority_order:
            ...
            if provider.is_available():
                ...
                suitable_models = [model for model in provider.supported_models if model in self.MODEL_CAPABILITIES]
    ...
    async def execute_with_fallback(
        self, messages: List[Message], routing_decision: RoutingDecision
    ) -> CompletionResponse:
        """Execute request with automatic fallback on failure."""
        # Try primary provider
        try:
            response = await routing_decision.provider.complete(messages, routing_decision.model_config)
            return response
            ...
            for fallback_provider, fallback_config in routing_decision.fallback_options:
                try:
                    response = await fallback_provider.complete(messages, fallback_config)
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        stats = {"cache_size": len(self._routing_cache), "providers": {}}
